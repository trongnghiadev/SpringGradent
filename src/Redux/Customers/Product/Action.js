...
export const activeProduct = (productId, status) => async (dispatch) => {
  try {
    dispatch({ type: ACTIVE_PRODUCT_REQUEST });

    const { data } = await api.put(
      `${API_BASE_URL}/api/admin/products/${productId}/active/${status}`,
      { status }
    );

    dispatch({
      type: ACTIVE_PRODUCT_SUCCESS,
      payload: data,
    });
  } catch (error) {
    dispatch({
      type: ACTIVE_PRODUCT_FAILURE,
      payload:
        error.response && error.response.data.message
          ? error.response.data.message
          : error.message,
